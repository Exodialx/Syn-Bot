...
export function calcEconomyHealth(): {
  ...
  for (const p of players) {
    ...
    // soft target net worth by progression
    estimated += 40000 + lvl * 28000 + classL * 55000 + Math.min(lvl, 50) * 12000 + Math.min(lvl, 100) * 8000;
  }
  // floor estimated to avoid div zero weirdness
  estimated = Math.max(estimated, n * 75000);
