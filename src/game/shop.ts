...
import { getHearts, setHearts, isHospitalized, heartsBar } from './health.js';
import { resetLiveBoostQuota, LIVE_BOOST_DAILY_LIMIT } from './ai.js';
...
export const SHOP: ItemDef[] = [
  ...
  { id: 'heartshot', name: 'Heart Shot', icon: '💉', slot: 'premium', cost: 1000000, desc: 'Restores 1 heart · instantly clears hospital' },
  { id: 'aitoken', name: 'AI Token', icon: '🧠', slot: 'premium', cost: 1000000, desc: `Refills SynAI live boost back to ${LIVE_BOOST_DAILY_LIMIT}/day` },
...
export function buyItem(p: Player, id: string): string {
  ...
  // Heart Shot — instant consumable, never goes into inventory
  if (item.id === 'heartshot') {
    const q = getDynamicPrice(item, p);
    if (p.cash < q.price) {
      return `❌ Need $${q.price.toLocaleString()} for Heart Shot`;
    }
    const wasHosp = isHospitalized(p);
    const before = getHearts(p);
    p.cash -= q.price;
    setHearts(p, before + 1);
    (p as any).hospitalUntil = 0;
    recordSale(item.id);
    savePlayer(p);
    const freed = wasHosp ? '\n🏥 Hospital cleared — you\'re free.' : '';
    return `💉 *Heart Shot* used
❤️ Hearts ${before} → ${getHearts(p)}  ${heartsBar(p)}
💸 -$${q.price.toLocaleString()}
💰 Cash $${p.cash.toLocaleString()}${freed}`;
  }

  // AI Token — instant consumable, never goes into inventory
  if (item.id === 'aitoken') {
    const q = getDynamicPrice(item, p);
    if (p.cash < q.price) {
      return `❌ Need $${q.price.toLocaleString()} for an AI Token`;
    }
    p.cash -= q.price;
    resetLiveBoostQuota(p.id);
    recordSale(item.id);
    savePlayer(p);
    return `🧠 *AI Token* used
🔮 SynAI live boost refilled → ${LIVE_BOOST_DAILY_LIMIT}/${LIVE_BOOST_DAILY_LIMIT}
💸 -$${q.price.toLocaleString()}
