...
import { dataStatus, forceBackup, getDbPath } from './db/database.js';
import {
  formatPlatformMenu,
  configureGroup,
  commandAllowed,
...
setMessageHandler(async (chatJid, senderId, text, raw, sock) => {
  ...
  if (cmd === '.sticker' || cmd === '.s') {
    try {
      ...
      // unwrap view-once
      const vo = quoted?.viewOnceMessage?.message || quoted?.viewOnceMessageV2?.message || msg.viewOnceMessage?.message;
      if (!mediaMsg && vo) {
        mediaMsg = vo.imageMessage || vo.stickerMessage;
      }
      if (!mediaMsg) {
        await sendText(chatJid, '❌ Reply to a *photo* with `.sticker`\n(Compressed images work best — not view-once)', raw);
        return;
      }

      let buf: Buffer | null = null;
      // 1) try downloadMediaMessage on a synthetic WAMessage
      try {
        const synthetic: any = {
          key: raw.key,
          message: quoted
        ...
        if (typeof downloadMediaMessage === 'function') {
          ...
          if (out && Buffer.isBuffer(out) && out.length > 100) buf = out;
        }
      } catch (e) {
        console.error('downloadMediaMessage', e);
      }

      // 2) stream fallback
      if (!buf) {
        try {
          const kind = mediaMsg.mimetype?.includes('webp') ? 'sticker' : 'image';
          const stream = await downloadContentFromMessage(mediaMsg, kind as any);
          const chunks: Buffer[] = [];
          for await (const chunk of stream) chunks.push(Buffer.from(chunk));
          buf = Buffer.concat(chunks);
        } catch (e2) {
          console.error('downloadContentFromMessage', e2);
        }
      }
      ...
      if (!buf || buf.length < 100) {
        await sendText(
          chatJid,
          '❌ Media not downloadable.\nTips:\n• Reply to a normal *photo* (not view-once)\n• Re-send the image, then reply `.sticker`\n• Avoid restricted forwards',
          raw
        );
        return;
      }
      ...
      try {
        await sock.sendMessage(chatJid, { sticker: buf }, { quoted: raw });
      } catch {
        await sock.sendMessage(
          chatJid,
          { image: buf, caption: '📎 Sent as image (sticker encode failed)' },
          { quoted: raw }
        );
      }
    } catch (e: any) {
